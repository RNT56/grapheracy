...
- The web decomposition now keeps HTTP/view-model contracts and provider/settings metadata in `workspaceTypes.ts`;
  ...
  `SettingsWorkspace.tsx`; planning sessions, build specs, tool traces, artifacts, and research actions live in
...
- The manual `external-canaries` protected workflow pulls the eight commit-bound manifests already published and
  exercised by staging, verifies the artifact commit, registry digests, and staging-workflow Sigstore identities, pins
  the disposable stack to those digests, then runs real GitHub, Google Drive, Notion, OpenAI, GitHub Issue, SMTP, and
  HMAC workflow adapters. Its receipt includes the commit, staging run, candidate-manifest SHA-256, eight immutable
  digests, and only redacted target/job/run/outcome IDs; the workflow closes the GitHub receipt issue, deletes stored
  action/provider credentials, and destroys the stack. A names-only manager validates, configures, reports, and
  dispatches the protected environment without printing secret values. The repository environment requires approval
  from `RNT56`; its protected secrets and fixture variables are currently empty, so the three external rows remain
  explicitly blocked rather than being represented as production-proven. Main branch protection requires its
  `Secret-backed connector, AI, and action canaries` check in addition to the exact-image staging, live-stack,
  migration, browser, documentation, security, and image gates.
