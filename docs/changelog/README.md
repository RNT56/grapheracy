...
Unreleased fragments live in `unreleased/` and are consolidated into the root `CHANGELOG.md` during release. The
directory is intentionally empty immediately after a version is cut; the next meaningful change starts the next
fragment set.
...
- A stable workspace version must have no remaining fragments and a matching version section in `CHANGELOG.md`.
